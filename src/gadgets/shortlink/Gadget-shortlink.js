...
$(() => {
    const {
        ...
        wgScriptPath,
        skin,
    } = mw.config.get(["wgArticleId", "wgCurRevisionId", "wgRevisionId", "wgDiffOldId", "wgDiffNewId", "wgServer", "wgScriptPath", "skin"]);
    if (wgArticleId <= 0) {
        return;
    }
    ...
    const markStatus = (ele, status, result) => {
        ele.innerText = status ? `${ele.dataset.type}${TEXT_COPY}${result ? TEXT_COPY_SUCCESS : TEXT_COPY_FAILED}` : `${TEXT_COPY}${ele.dataset.type}`;
    };
    ...
    switch (skin) {
        ...
        case "moeskin": {
            ...
            $("#p-sl h3").after(
                '<div style="display:flex"><div style="width:0.25rem;border-radius:99em;background:rgba(0,0,0,0.102);margin-right:1rem"></div><ul id="p-sl-list" style="list-style:none"></ul></div>',
            );
            ...
            for (const shortlink of links) {
                ...
                $item.append(
                    `<div>（<a href="javascript:void(0)" data-copy-content="${shortlink.wikitext}" data-type="wikitext"></a><wbr>丨<a href="javascript:void(0)" data-copy-content="${wgServer}${wgScriptPath}/_?${shortlink.href}" data-type="${wgULS("短链接", "短網址")}"></a>）</div>`,
                );
            ...
            if (portletLink) {
                portletLink.querySelector("a").addEventListener("click", (e) => {
                    ...
                    links.forEach((shortlink) => {
                        $element.append($("<div>").css({ "font-weight": "bold", margin: "0.6em 0 0.2em" }).text(shortlink.title));
