...
$(() =>
    (async () => {
        ...
        class ACUPWindow extends OO.ui.ProcessDialog {
            ...
            initialize() {
                ...
                this.confirmPanel.$element.append(
                    `<div>${wgULS("确认获取参与讨论的自动确认用户列表？（该操作将对每位可能用户发出一次查询贡献的API请求，请谨慎使用！）", "確認獲取參與討論的自動確認使用者列表？（該操作將對每位可能使用者發出一次查詢貢獻的API請求，請謹慎使用！）")}</div>`,
                );
            ...
            async getPing() {
                this.confirmPanel.$element
                    .empty()
                    .append([`<p>${wgULS("为确保剪贴板复制成功，请不要将鼠标移动出页面或切换窗口！", "為確保剪貼簿複製成功，請不要將滑鼠移動出頁面或切換視窗！")}</p>`, this.stepList]);
                this.updateSize();
                ...
                const validAC = await chunkify(nonMGUsers, (hasHighLimit && 500) || undefined).reduce(async (acc, chunk, i) => {
                    ...
                    const prelimRes = (
                        ...
                        .filter((u) => u.implicitgroups.includes("autoconfirmed") && !u.blockedby && moment(pageCreationTime).diff(moment(u.registration), "days") > 33)
                        .map((u) => u.name);
                    console.log(`[ACUserPing] Chunk ${i + 1}: Got preliminary result.`, prelimRes);

                    const lastCheck = await Promise.all(
                        prelimRes.map(async (u) => {
                            await sleep();
            ...
            async displayResults({ plain, ping }) {
                ...
                this.resultPanel.$element.empty().append([
                    $("<div>", {
                        css: {
                            display: "flex",
                        },
        ...
        $(mw.util.addPortletLink("p-tb", "#", wgULS("获取发言列表", "獲取發言列表"), "t-lr-ACUserPing", wgULS("获取参与讨论的自动确认用户列表", "獲取參與討論的自動確認使用者列表"))).on(
            "click",
            (e) => {
                e.preventDefault();
