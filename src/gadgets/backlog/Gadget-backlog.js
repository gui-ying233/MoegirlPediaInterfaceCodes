...
$(() =>
    (async () => {
        ...
        try {
            ...
            const blockLogResult = await autoRetryAsyncFunction(
                async () =>
                    await api.post({
                        action: "query",
            ...
            if (blockLogResult && blockLogResult.query && blockLogResult.query.logevents) {
                ...
                if (blocklogevents.length > 0) {
                    ...
                    blocklogeventsContainer.html(
                        "<thead><tr><th>时间</th><th>执行者</th><th>目标</th><th>类型</th><th>结束时间</th><th>额外限制</th><th>备注</th><th>操作</th></tr></thead><tbody></tbody>",
                    );
                    ...
                    blocklogeventTemplate.html(
                        `${'<td class="style-justify"></td><td class="style-justify userColumn"></td>'}${'<td class="style-justify"></td>'.repeat(3)}<td></td><td class="word-break-all"></td><td class="style-justify"></td>`,
                    );
                    blocklogevents.forEach(({ logid, action, actionhidden, suppressed, title, user, timestamp, tags, parsedcomment, params, userhidden }) => {
                        const { duration: _duration, expiry, flags: _flags } = params || {};
                        ...
                        [
                            ...
                            isIPAddressOrRange || loghidden
                                ? `<span title="${_target}">${target}</span>`
                            ...
                            flags
                                ? flags.length > 0
                                    ? `<ul class="blocklogevents-ul">${flags.map((f) => `<li class="blocklogevents-flags-${f}" ${blocklogFlags[f] ? "" : 'style="font-style: italic;"'} title="${f}">${blocklogFlags[f] || f}</li>`).join("")}</ul>`
                                    : "（无）"
                                : "-",
                            ...
                            (action === "unblock"
                                ...
                                : loghidden
                                  ? loghiddenMsg
                                  : `<span class="mw-logevent-actionlink"><a href="/Special:%E8%A7%A3%E9%99%A4%E5%B0%81%E7%A6%81/${encodeURIComponent(target)}" title="Special:解除封禁/${target}">解封</a> | <a href="/Special:%E5%B0%81%E7%A6%81/${encodeURIComponent(target)}" title="Special:封禁/${target}">更改封禁</a></span>`) +
                                (hasDeletelogentryRight
                                    ? `<hr style="margin: 2px 7px;"><a href="/index.php?action=historysubmit&type=logging&revisiondelete=1&ids%5B${encodeURIComponent(logid)}%5D=1" title="删除/还原版本">显示/隐藏选择的版本</a>`
                                    : ""),
                        ].forEach((data, index) => {
            ...
            const abuseLogResult = await autoRetryAsyncFunction(
                async () =>
                    await api.post({
                        action: "query",
                        assertuser: wgUserName,
            ...
            if (abuseLogResult && abuseLogResult.query && abuseLogResult.query.abuselog) {
                ...
                const abuselogevents = abuseLogResult.query.abuselog
                ...
                if (abuselogevents.length > 0) {
                    ...
                    abuselogeventsContainerRoot.append(
                        `<div class="abuselogevents-table-caption" title="第一条日志ID：${veryFirstAbuselogId}">最近500条滥用日志中最新10条带有“禁止自动授权、从用户组移除、封禁”中部分或全部设定的过滤器的日志 [<a id="expand-abuselogevents">扩展/收缩列表</a>]</div>`,
                    );
                    const abuselogeventsContainer = $("<table/>");
                    abuselogeventsContainer.addClass("wikitable abuselogevents-table");
                    abuselogeventsContainer.html(
                        "<thead><tr><th>时间</th><th>用户</th><th>页面</th><th>操作</th><th>过滤器</th><th>过滤器描述</th><th>采取的行动</th><th>操作</th></tr></thead><tbody></tbody>",
                    );
                    ...
                    abuselogevents.forEach(({ action: _action, filter: description, filter_id: filterID, id, result: _result, timestamp, title, user }) => {
            ...
            result.forEach(({ domain, cmtitle, count }, index) => {
                ...
                self.html(
                    `<a href="https://${domain}/${encodeURIComponent(cmtitle)}" target="_blank" rel="nofollow noreferrer noopener" class="external text">${domain.match(/^[^.]+/)[0]}:${cmtitle}: ${count}页面</a>`,
                );
                if (count > 0) {
            ...
            if (reason instanceof Error) {
                errorInfoNode.html(
                    `<p>日志查核工具发生错误：</p><dl><dt>错误类型：</dt><dd>${$("<span/>").text(reason.name).html()}</dd><dt>错误信息：</dt><dd><pre>${$("<span/>")
                        .text(`${reason.toString()}\n${reason.stack.split("\n").slice(1).join("\n")}`)
                        .html()}</pre></dd></dl>`,
                );
            } else {
                errorInfoNode.html(
                    `<p>日志查核工具发生错误：</p><pre>${$("<span/>")
                        .text(JSON.stringify(reason, undefined, 4))
                        .html()}</pre>`,
                );
            }
        } finally {
