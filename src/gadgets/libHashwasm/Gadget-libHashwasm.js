    ...
    (function (global) {
        !function () {
            (Object.defineProperty(_$indexEsm_2, "__esModule", {
                ...
                (_$indexEsm_2.createSHA256 = createSHA256),
                (_$indexEsm_2.createSHA3 = function (bits = 512) {
                ...
                (_$indexEsm_2.sha3 = function (data, bits = 512) {
                    ...
                    if (null === wasmCache$c || wasmCache$c.hashLength !== hashLength)
                        return lockedCreate(mutex$c, wasmJson$b, hashLength).then((wasm) => (wasmCache$c = wasm).calculate(data, bits, 6));
                    try {
            ...
            let nodeBuffer = null != (_a = globalObject.Buffer) ? _a : null,
            ...
            function getDecodeBase64Length(data) {
                let bufferLength = Math.floor(0.75 * data.length);
                var len = data.length;
            ...
            function decodeBase64(data) {
                ...
                for (let i = 0; i < len; i += 4) {
                    var encoded1 = base64Lookup[data.charCodeAt(i)],
                        encoded2 = base64Lookup[data.charCodeAt(i + 1)],
                        encoded3 = base64Lookup[data.charCodeAt(i + 2)],
                        encoded4 = base64Lookup[data.charCodeAt(i + 3)];
                    ((bytes[p] = (encoded1 << 2) | (encoded2 >> 4)),
                        (bytes[(p += 1)] = ((15 & encoded2) << 4) | (encoded3 >> 2)),
                        (bytes[(p += 1)] = ((3 & encoded3) << 6) | (63 & encoded4)),
                        (p += 1));
                }
                return bytes;
            }
            ...
            function WASMInterface(binary, hashLength) {
                return __awaiter(this, void 0, void 0, function* () {
                    ...
                    let getStateSize = () => new DataView(wasmInstance.exports.memory.buffer).getUint32(wasmInstance.exports.STATE_SIZE, !0),
                        loadWASMPromise = wasmMutex.dispatch(() =>
                    ...
                    let init = (bits = null) => {
                        ...
                        digestChars = new Uint8Array(2 * hashLength),
                        digest = (outputType, padding = null) => {
                            if (initialized)
                                return (
                                    (initialized = !1),
                                    wasmInstance.exports.Hash_Final(padding),
                                    "binary" === outputType ? memoryView.slice(0, hashLength) : getDigestHex(digestChars, memoryView, hashLength)
                                );
                            throw new Error("digest() called before init()");
                        };
                    let isDataShort = (data) => ("string" == typeof data ? data.length < MAX_HEAP / 4 : data.byteLength < MAX_HEAP),
                    ...
                    return (
                        ...
                        }),
                        {
                            ...
                            save: () => {
                                ...
                                if (initialized)
                                    return (
                                        (stateOffset = wasmInstance.exports.Hash_GetState()),
                                        (stateLength = getStateSize()),
                                        (memoryBuffer = wasmInstance.exports.memory.buffer),
                                        (memoryBuffer = new Uint8Array(memoryBuffer, stateOffset, stateLength)),
                                        writeHexToUInt8((stateOffset = new Uint8Array(WASM_FUNC_HASH_LENGTH + stateLength)), binary.hash),
                                        stateOffset.set(memoryBuffer, WASM_FUNC_HASH_LENGTH),
                                        stateOffset
                                    );
                            ...
                            load: (state) => {
                                ...
                                var stateOffset = wasmInstance.exports.Hash_GetState(),
                                    stateLength = getStateSize(),
                                    overallLength = WASM_FUNC_HASH_LENGTH + stateLength,
                                    memoryBuffer = wasmInstance.exports.memory.buffer;
                                ...
                                if (
                                    ...
                                    })(binary.hash, state.subarray(0, WASM_FUNC_HASH_LENGTH))
                                )
                            ...
                            calculate: (data, initParam = null, digestParam = null) => {
                                var buffer;
                                return canSimplify(data, initParam)
                                    ? ((buffer = getUInt8Buffer(data)),
                                      memoryView.set(buffer),
                                      wasmInstance.exports.Hash_Calculate(buffer.length, initParam, digestParam),
                                      getDigestHex(digestChars, memoryView, hashLength))
                                    : (init(initParam), update(data), digest("hex", digestParam));
                            },
            ...
            function lockedCreate(mutex, binary, hashLength) {
                return __awaiter(this, void 0, void 0, function* () {
                    var unlock = yield mutex.lock(),
                        wasm = yield WASMInterface(binary, hashLength);
                    return (unlock(), wasm);
                });
            }
            new Mutex();
            ...
            let mutex$a = new Mutex(),
                wasmCache$a = null;
            let mutex$9 = new Mutex(),
                wasmCache$9 = null;
            ...
            let mutex$8 = new Mutex(),
                wasmCache$8 = null;
            let mutex$7 = new Mutex(),
                wasmCache$7 = null;
            new Mutex();
            (new Mutex(), new Uint8Array(8));
            (new Mutex(), new Uint8Array(8));
