...
$(() =>
    (async () => {
        ...
        const api = new mw.Api(),
            zhAPI = /m?zh\.moegirl\.org\.cn/.test(location.hostname) ? api : new mw.ForeignApi("https://mzh.moegirl.org.cn/api.php", { anonymous: true });
        ...
        const escapeWikitext = (original) => {
            ...
            replaced = replaced.replace(REGEXP.link, (match, target, anchor, text) =>
                anchor || text ? `[[${text ? mappings.push(target) - 1 : target}${anchor ? `#${mappings.push(anchor) - 1}` : ""}${text || ""}]]` : match,
            );
        ...
        const restoreWikitext = (original, mappings) => {
            ...
            replaced = replaced.replace(REGEXP.link, (match, target, anchor, text) =>
                anchor || text ? `[[${text ? mappings[target] : target}${mappings[anchor] ? `#${mappings[anchor]}` : ""}${text || ""}]]` : match,
            );
            // Restore nowiki
        ...
        $(mw.util.addPortletLink("p-cactions", "#", wgULS("自动繁简转换", "自動繁簡轉換"), "ca-VariantConverter", wgULS("自动同步界面消息的繁简版本", "自動同步介面消息的繁簡版本"))).on(
            "click",
            (e) => {
                e.preventDefault();
