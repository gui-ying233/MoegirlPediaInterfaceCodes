...
$(() =>
    (async () => {
        ...
        const generatePageLinkSelector = (title) =>
            deduplicate([encodeURI(title), mw.util.wikiUrlencode(title)])
        ...
        const $root = $(".mw-category-generated"),
            $items = $root.find("li");
        const $control = $("<p id='batdel-control'>");
        const $portlet = $(
                mw.util.addPortletLink("p-cactions", "#", wgULS("批量删除本分类下页面", "批次刪除本分類下頁面"), "ca-batdel", wgULS("批量删除本分类下页面", "批次刪除本分類下頁面")),
            ).addClass("sysop-show"),
            $portletAnchor = $portlet.find("a");
        const pages = [];
            ...
            for (const {
                ...
                for (let retryTimes = 0; retryTimes < 3; retryTimes++) {
                    try {
                        ...
                        const $reason = $html.find("#reason"),
                            $actor = $html.find("#actor a").first();
                        const reason = $reason.text().trim(),
                        ...
                        if ($reason.length === 1 && $actor.length === 1 && reason && actor) {
                            ...
                            if (isTrusted) {
                                // Flag is trusted
                                link.after(
                                    `<div>${wgULS("挂删人", "掛刪人")}：<a href="/User:${user}" class="mw-userlink batdel-bypass"><bdi>${user}</bdi></a></div><div>${wgULS("挂删理由", "掛刪理由")}：${reason}</div>`,
                                );
                            } else {
                                // Flag is not trusted, do not delete
                                link.prop("target", "_blank").after(
                                    `<div class="batdel-error">${wgULS("禁止删除：该次挂删不可靠，请手动检查", "禁止刪除：該次掛刪不可靠，請手動檢查")}（${user !== $actor.text() ? wgULS("最后编辑者与挂删人不符", "最後編輯者與掛刪人不符") : wgULS("最后编辑者没有巡查权限", "最後編輯者沒有巡查權限")}）</div>`,
                                );
                            ...
                            link.addClass("batdel-bypass")
                                .prop("target", "_blank")
                                .after(
                                    `<div class="batdel-error">${wgULS("禁止删除：该次挂删不可靠，请手动检查（挂删模板未给出理由或挂删人）", "禁止刪除：該次掛刪不可靠，請手動檢查（掛刪模板未給出理由或掛刪人）")}</div>`,
                                );
        ...
        $portlet.on("click", () => {
            ...
            $control
                .empty()
                .append([`${wgULS("请选择要删除的页面", "請選擇要刪除的頁面")} [`, selectedNum, "/", totalNum, "] ", toggleSelection, runDeletion, cancelDeletion])
                .prependTo($root);
            $("body").addClass("batdel-body");

            // Add checkboxes
            $items
                .each((_, ele) => $(ele).prepend($("<input type='checkbox' class='batdel-select'>").prop("disabled", $(ele).find(".batdel-error")[0])))
                .find(".stub")
                .toggleClass("stub _stub");
            const checkboxes = $items.find(".batdel-select:not(:disabled)");
            totalNum.text(checkboxes.length);
            checkboxes.on("change", () => selectedNum.text(checkboxes.filter(":checked").length));
            $root
                .children("div")
                .children("p")
                .each((_, ele) => {
                    $(`<button class="batdel-controlButton">${wgULS("全选/全不选本类别页面", "全選/全不選本類別頁面")}</button>`)
                        .on("click", (e) =>
                            $(e.target)
                                .closest(".mw-category-generated > div")
                                .find(".batdel-select:not(:disabled)")
                                .each((_, ele) => $(ele).prop("checked", !ele.checked))
                                .trigger("change"),
                        )
                        .appendTo(ele);
                });
            ...
            runDeletion.on("click", async () => {
                if (
                    ...
                    !(await oouiDialog.confirm(
                        `${wgULS("您确定要删除这些页面吗？", "您確定要刪除這些頁面嗎？")}（${wgULS("选中了", "選中了")}${$items.find(".batdel-select:checked").length}${wgULS("个页面", "個頁面")}）`,
                        {
                ...
                const $spinner = $('<img src="https://img.moegirl.org.cn/common/d/d1/Windows_10_loading.gif" style="height: 1em; margin-top: -.25em;">'),
                    $status = $("<span>");
                ...
                try {
                    ...
                    for (const ele of $root.find("a").not(".batdel-bypass, .batdel-disabled").toArray()) {
                        ...
                        try {
                            ...
                            self.after(
                                `<span class="batdel-result batdel-error"> ${wgULS("删除失败", "刪除失敗")}：${e instanceof Error ? `${e} ${e.stack.split("\n")[1].trim()}` : JSON.stringify(e)}</span>`,
                            );
                        }
                    }
                    $spinner.remove();
