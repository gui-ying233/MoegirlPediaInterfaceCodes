...
(async () => {
    ...
    if (wgCanonicalSpecialPageName !== "Contributions") {
        ...
        console.log(revids);
        // 过滤掉已经存储过的更改，再请求
        const promises = sliceRevids(revids.filter((rid) => !statusCache.some(({ revid }) => +rid === +revid))).map(queryModerationStatus);
        ...
        const {
            ...
            format: "json",
            prop: "revisions",
            revids: [$contributionsList.get(0).dataset.mwRevid, $contributionsList.get(-1).dataset.mwRevid],
            rvprop: ["timestamp"],
        });
