...
$(() => {
    ...
    if (mw.config.get("wgCanonicalSpecialPageName") === "Userrights") {
        ...
        Array.from(wpExpiry.options)
            .filter((ele) => ele.value === "1 day")[0]
            .before(new Option("30分钟", "30 minutes"), new Option("2小时", "2 hours"), new Option("6小时", "6 hours"));
    }
    // 替换文本默认不勾选「通过Special:最近更改和监视列表通知这些编辑」
    if (mw.config.get("wgCanonicalSpecialPageName") === "ReplaceText" && $("#powersearch")[0]) {
        $("#mw-search-ns0").prop("checked", false);
        $('input[name="botEdit"], #mw-search-ns6').prop("checked", true);
    }
});
