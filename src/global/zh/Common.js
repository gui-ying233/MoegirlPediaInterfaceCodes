...
(async () => {
    ...
    XMLHttpRequest.prototype.getResponseHeader = function (name) {
        return `\n${this.getAllResponseHeaders().toLowerCase()}`.includes(`\n${name.toLowerCase()}: `)
            ? getResponseHeader.bind(this)(name)
            : (console.info(`Refused to get unsafe header "${name}"\n`, this, "\n", new Error().stack), null);
    };
                                ...
                                if ($(e.target).is("img")) {
                                    window
                                        .open(
                                            $(e.target)
                                                .attr("src")
                                                .replace(/(img\.moegirl\.org\.cn\/common\/)thumb\/([a-z\d]+\/[a-z\d]+\/)([^/]+)\/\d+px-\3/i, "$1$2$3"),
                                            "_blank",
                                        )
                                        .focus();
                                    e.preventDefault();
    ...
    const copyRights = async () => {
        ...
        $("#mw-content-text").on("copy", () => {
            ...
            if (
                !value.length ||
                value.length < 128 || // 当复制内容为空或长度小于定值时不添加声明
